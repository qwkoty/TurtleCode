...
import { useTurtleCodeStore, AgentStatus, Attachment } from "@/lib/store";
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* 左侧聊天 */}
        <section className="flex w-[70%] flex-col border-r border-slate-700/30">
          {/* 顶部栏 */}
          ...
          {/* 底部输入 */}
          <div className="border-t border-slate-700/30 bg-slate-900/40 p-4">
            ...
            <div className="flex items-end gap-2 rounded-2xl border border-slate-700/50 bg-slate-900/60 p-2 focus-within:border-brand-primary/60 focus-within:ring-1 focus-within:ring-brand-primary/30">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={handleSend}
                  className="ml-1 flex h-8 w-8 items-center justify-center rounded-xl bg-brand-primary text-white transition-transform hover:scale-105 active:scale-95"
                >
            ...
            <div className="mt-2 flex items-center justify-between text-[10px] text-slate-500">
              <div className="flex gap-3">
                <span className="flex items-center gap-1">
                  <Zap className="h-3 w-3 text-brand-highlight" /> Token {tokenUsage}
                </span>
                <span className="flex items-center gap-1">
                  <Coins className="h-3 w-3 text-emerald-400" /> ${cost.toFixed(4)}
                </span>
                <span className="flex items-center gap-1">
                  <BarChart3 className="h-3 w-3 text-brand-accent" /> 缓存 {cacheHitRate}%
                </span>
              </div>
        ...
        {/* 右侧代理工作区 */}
        <section className="flex w-[30%] flex-col bg-slate-900/20">
          {/* 当前编辑文件 */}
          ...
          {/* Diff 查看器 */}
          <div className="flex flex-1 flex-col border-b border-slate-700/30 p-4">
            <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
      ...
      {/* 底部状态栏 */}
      <footer className="flex h-8 items-center justify-between border-t border-slate-700/30 bg-slate-950/60 px-4 text-[11px] text-slate-400">
        <div className="flex items-center gap-4">
          ...
          <span className="flex items-center gap-1">
            <BarChart3 className="h-3 w-3" /> 缓存 {cacheHitRate}%
          </span>
        </div>
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5">
